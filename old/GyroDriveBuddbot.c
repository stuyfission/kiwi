...
/*
 ...
 As of right now, this code is currently (or being trivially adapted for) capable of attitude control (i.e. the ability to hold the
...
task main() {
  ...
  int vals[200];
  int iter = 0;
  wait10Msec(100); // Allow the gyro to initialize...

  while (iter < 200) {
    vals[iter] = SensorValue[S4]; // Read in gyro values while at rest, every 10 milliseconds
    wait10Msec(1);
    iter++;
  }
  ...
  int sum = 0;
  int div = 0;
  for(iter = 0; iter < 4; iter++) { // Find the arithmetic mean of the array
   sum += vals[iter];
  }

        nxtDisplayTextLine(2, "%d %d\n", vals[0], sum); // Debug line
        wait10Msec(150); // Debug
        int offset;
        offset = (int)(sum / 4);
        nxtDisplayTextLine(2, "%d %d\n", sum, offset); // Debug line
